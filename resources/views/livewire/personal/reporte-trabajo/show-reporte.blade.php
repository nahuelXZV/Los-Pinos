    ...
    <x-table>
        <div class=" px-4 py-6 flex items-center">
            ...
            <a class="font-bold text-white rounded cursor-pointer bg-green-600 hover:bg-green-500 py-2 px-4 "
                @if ($search == '')
                href="{{ route('reporteTrabajoLista.pdf', ['_@_', $sort, $direction]) }}"
            @else
                href="{{ route('reporteTrabajoLista.pdf', [$search, $sort, $direction]) }}"
                @endif>
                    ...
                    @foreach ($reportes as $reporte)
                        <tr>
                            ...
                            <td class="my-3 px-6 py-4 whitespace-nowrap flex">
                                ...
                                <a class="font-bold text-white rounded cursor-pointer bg-green-600 hover:bg-green-500 py-2 px-4 "
                                    href="{{ route('reporteTrabajo.pdf', $reporte->id) }}">
                                ...
                                @if (auth()->user()->can('reporteTrabajo.show') ||
    auth()->user()->can('reporteTrabajo.delete'))
                                    @can('reporteTrabajo.show')
                                        <a class="ml-2 font-bold text-white rounded cursor-pointer bg-blue-600 hover:bg-blue-500 py-2 px-4"
                                            href=" {{ route('reporteTrabajo.show', $reporte->id) }}">
                                            <svg xmlns=" http://www.w3.org/2000/svg" class="h-6 w-6" fill="none"
                                                viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
